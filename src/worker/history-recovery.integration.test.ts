...
function requireDisposableEnvironment() {
  ...
  if (!databaseUrl.includes("localhost:5434/history_verification")) {
    throw new Error("integration test requires disposable PostgreSQL database localhost:5434/history_verification");
  }
  if (!redisUrl.includes("localhost:6380")) {
    throw new Error("integration test requires disposable Redis localhost:6380");
  }
}
...
integrationTest("PostgreSQL/Redis recovery barriers survive crash, replay, loss, and invalid input", async (t) => {
  ...
  try {
    await t.test("worker crash after record commit resumes idempotently before barriers", async () => {
      ...
      const envelope = buildRecordEnvelope({
        ...
        eventKey: "deal:1",
        payload: {
          ticket: 1, order: 2, positionId: null, symbol: "EURUSD", type: "buy", direction: "in",
          volume: 0.1, price: 1.1, commission: 0, fee: 0, swap: 0, profit: 1,
          balanceAfter: 1001, time: 946684900, comment: "",
        },
      });
      const workerA = new PrismaClient();
      await persistHistoryRecord(workerA as never, account.id, accountNo, "deals", envelope, 180);
      await workerA.$disconnect();

      const before = await admin.bridgeHistoryCheckpoint.findUniqueOrThrow({ where: { tradingAccountId: account.id } });
      assert.equal(String(before.completedThroughServerTime), START);
      const workerB = new PrismaClient();
      await persistHistoryRecord(workerB as never, account.id, accountNo, "deals", envelope, 180);
      const digest = nextRecordsSha256(EMPTY_RECORDS_SHA256, envelope.payloadSha256);
      await persistHistoryBarrier(workerB as never, account.id, barrier(accountNo, chunkId, "deals", {
        recordCount: 1, recordsSha256: digest, dealCursor: envelope.dealCursor,
      }), 180);
      await persistHistoryBarrier(workerB as never, account.id, barrier(accountNo, chunkId, "orders", {
        dealCursor: envelope.dealCursor,
      }), 180);
      const committed = await persistHistoryBarrier(workerB as never, account.id, barrier(accountNo, chunkId, "position-closed", {
        dealCursor: envelope.dealCursor,
      }), 180);
    ...
    await t.test("database commit survives lost Redis acknowledgement and Redis flush", async () => {
      ...
      await persistHistoryBarrier(admin as never, account.id, barrier(accountNo, chunkId, "orders"), 180);
      await persistHistoryBarrier(admin as never, account.id, barrier(accountNo, chunkId, "position-closed"), 180);
      assert.equal(await redis.exists(ackKey), 0);

      const durable = await ensureHistoryCheckpoint(admin as never, account.id);
      await mirrorHistoryCheckpoint(redis, accountNo, durable);
      assert.equal(JSON.parse((await redis.get(ackKey))!).lastCompletedChunkId, chunkId);

      await redis.flushDb();
      assert.equal(await redis.exists(ackKey), 0);
      await mirrorHistoryCheckpoint(redis, accountNo, await ensureHistoryCheckpoint(admin as never, account.id));
      assert.equal(JSON.parse((await redis.get(ackKey))!).lastCompletedChunkId, chunkId);
      assert.equal(JSON.parse((await redis.get(cursorKey))!).deals.time, START);
    });

    await t.test("empty Deal and Order windows create gap-free durable coverage", async () => {
      const { account, accountNo } = await fixture("empty");
      const chunkId = randomUUID();
      await persistHistoryBarrier(admin as never, account.id, barrier(accountNo, chunkId, "deals"), 180);
      await persistHistoryBarrier(admin as never, account.id, barrier(accountNo, chunkId, "orders"), 180);
      const committed = await persistHistoryBarrier(admin as never, account.id, barrier(accountNo, chunkId, "position-closed", {
        reconstructionState: {
          pending: { "101": [{ ticket: 1, time: 946684900 }] },
          orders: { "2": { ticket: 2, time_setup: 946684850, time_done: 946684950 } },
          volumes: { "101": 0.1 },
        },
      }), 180);
    ...
    await t.test("duplicate record and barrier replay stays idempotent", async () => {
      ...
      const chunkId = randomUUID();
      const envelope = buildRecordEnvelope({
        accountNo, stream: "deals", chunkId, parentChunkId: null,
        windowStartServerTime: START, windowEndServerTime: END,
        dealCursor: { time: "946684900", ticket: "7" }, orderCursor: { time: START, ticket: "0" },
        ordinal: 0, expectedCount: 1, eventKey: "deal:7",
        payload: {
          ticket: 7, order: 8, positionId: null, symbol: "EURUSD", type: "buy", direction: "in",
          volume: 0.1, price: 1.1, commission: 0, fee: 0, swap: 0, profit: 1,
          balanceAfter: 1001, time: 946684900, comment: "",
        },
      });
      await persistHistoryRecord(admin as never, account.id, accountNo, "deals", envelope, 180);
      await persistHistoryRecord(admin as never, account.id, accountNo, "deals", envelope, 180);
      const common = { dealCursor: envelope.dealCursor };
      const dealBarrier = barrier(accountNo, chunkId, "deals", {
        ...common, recordCount: 1, recordsSha256: nextRecordsSha256(EMPTY_RECORDS_SHA256, envelope.payloadSha256),
      });
      const orderBarrier = barrier(accountNo, chunkId, "orders", common);
      const positionBarrier = barrier(accountNo, chunkId, "position-closed", common);
      ...
      const first = await persistHistoryBarrier(admin as never, account.id, positionBarrier, 180);
      await persistHistoryBarrier(admin as never, account.id, dealBarrier, 180);
      await persistHistoryBarrier(admin as never, account.id, orderBarrier, 180);
    ...
    await t.test("digest/count mismatch and one-stream failure cannot advance checkpoint", async () => {
      const { account, accountNo } = await fixture("mismatch");
      const chunkId = randomUUID();
      await persistHistoryBarrier(admin as never, account.id, barrier(accountNo, chunkId, "deals"), 180);
      await assert.rejects(
        persistHistoryBarrier(admin as never, account.id, barrier(accountNo, chunkId, "orders", {
    ...
    await t.test("missing offset rejects barrier before checkpoint advancement", async () => {
    ...
    await t.test("completed backfill transitions to incremental exactly once", async () => {
      ...
      const positions = barrier(accountNo, chunkId, "position-closed", tail);
      ...
      const first = await persistHistoryBarrier(admin as never, account.id, positions, 180);
      await persistHistoryBarrier(admin as never, account.id, deals, 180);
      await persistHistoryBarrier(admin as never, account.id, orders, 180);
      const replay = await persistHistoryBarrier(admin as never, account.id, positions, 180);
    ...
    await t.test("checkpoint foreign keys target Account(id) and cascade", async () => {
      ...
      await assert.rejects(admin.bridgeHistoryCheckpoint.create({
