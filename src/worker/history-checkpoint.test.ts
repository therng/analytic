...
function barrier(stream: HistoryBarrier["stream"], overrides: Partial<HistoryBarrier> = {}): HistoryBarrier {
  return {
    version: 1,
    type: "barrier",
    accountNo: "acct-1",
...
test("one or two barriers cannot advance checkpoint", async () => {
  const state: { checkpoint: HistoryCheckpoint | null } = { checkpoint: createInitialHistoryCheckpoint("acct-1") };
...
test("reconstruction checkpoint preserves raw server time and adds UTC audit fields with shared converter", () => {
  const normalized = normalizeReconstructionState({
    ...
    volumes: { "101": 0.1 },
    orders: { "10": { ticket: 10, time_setup: 1_751_000_000, time_done: 1_751_000_060 } },
  }, 180) as any;

  assert.equal(normalized.pending["101"][0].time, 1_751_000_000);
  assert.equal(normalized.pending["101"][0].timeUtc, "2025-06-27T01:53:20.000Z");
  assert.equal(normalized.orders["10"].time_setup, 1_751_000_000);
  assert.equal(normalized.orders["10"].timeSetupUtc, "2025-06-27T01:53:20.000Z");
...
test("gap barrier fails without regressing or advancing checkpoint", async () => {
  const state: { checkpoint: HistoryCheckpoint | null } = { checkpoint: createInitialHistoryCheckpoint("acct-1") };
  await assert.rejects(() => applyHistoryBarrier(state, barrier("deals", {
...
function fakeDb() {
  ...
  const db: any = {
    ...
    bridgeHistoryChunk: {
      ...
      create: async ({ data }: any) => { const row = { ...data, dealsAppliedCount: 0, ordersAppliedCount: 0, positionsAppliedCount: 0, dealsAppliedDigest: EMPTY_RECORDS_SHA256, ordersAppliedDigest: EMPTY_RECORDS_SHA256, positionsAppliedDigest: EMPTY_RECORDS_SHA256, dealsBarrierAt: null, ordersBarrierAt: null, positionsBarrierAt: null, completedAt: null }; chunks.set(data.id, row); return row; },
    ...
    bridgeHistoryRecord: {
      findUnique: async ({ where }: any) => {
        ...
        return (db.receipts ?? new Map()).get(`${key.chunkId}:${key.stream}:${key.ordinal}`) ?? null;
      },
...
test("persistHistoryBarrier commits empty chunk only after all three stream barriers", async () => {
  ...
  const committed = await persistHistoryBarrier(db, "acct-1", barrier("position-closed", common), 0);
...
test("completed database checkpoint can rebuild a lost Redis acknowledgement on barrier replay", async () => {
  ...
  await persistHistoryBarrier(db, "acct-1", barrier("position-closed"), 0, "3-0");

  const recovered = await persistHistoryBarrier(db, "acct-1", barrier("position-closed"), 0, "4-0");
...
test("digest mismatch after one stream barrier cannot advance durable checkpoint", async () => {
  ...
  await assert.rejects(
    persistHistoryBarrier(fixture.db, "acct-1", barrier("orders", {
      recordsSha256: "0".repeat(64),
    }), 0),
    /count\/digest mismatch/,
  );
  assert.equal(fixture.checkpoint.lastCompletedChunkId, null);
  assert.equal(String(fixture.checkpoint.completedThroughServerTime), "946684800");
});

test("count mismatch cannot record barrier or advance checkpoint", async () => {
  const fixture = fakeDb();
  await assert.rejects(
    persistHistoryBarrier(fixture.db, "acct-1", barrier("deals", { recordCount: 1 }), 0),
    /count\/digest mismatch/,
  );
  assert.equal(fixture.checkpoint.lastCompletedChunkId, null);
});
...
test("automatic record persists idempotently before its barrier", async () => {
  ...
  const envelope = buildRecordEnvelope({
    ...
    eventKey: "deal:1", payload: {
      ticket: 1, order: 2, positionId: null, symbol: "EURUSD", type: "buy", volume: 0.1, price: 1.1,
      commission: 0, fee: 0, swap: 0, profit: 1, balanceAfter: 1001, time: 946684800, comment: "",
    },
  });
...
test("closed-position reconstruction writes identical once-converted UTC dates to Position and ClosedPosition", async () => {
  ...
  let closedArgs: any;
  db.position.upsert = async (args: any) => { positionArgs = args; };
  db.closedPosition.upsert = async (args: any) => { closedArgs = args; };
  const envelope = buildRecordEnvelope({
    accountNo: "123", stream: "position-closed", chunkId: "position-chunk", parentChunkId: null,
    windowStartServerTime: "946684800", windowEndServerTime: "949276800",
    dealCursor: { time: "946684900", ticket: "7" }, orderCursor: { time: "946684950", ticket: "8" },
    ordinal: 0, expectedCount: 1, eventKey: "position:9",
    payload: {
      ticket: 9, symbol: "EURUSD", positionType: 0, volume: 0.1,
      entryPrice: 1.1, exitPrice: 1.2, entryTime: 946684900, exitTime: 946685000,
      durationSeconds: 100, mae: -1, mfe: 2, profit: 10, commission: -1, swap: 0,
      dealTicket: 7, orderTicket: 8, comment: "",
    },
  });

  await persistHistoryRecord(db, "acct-1", "123", "position-closed", envelope, 180);

  assert.equal(positionArgs.create.openTime.toISOString(), "1999-12-31T21:01:40.000Z");
  assert.equal(positionArgs.create.closeTime.toISOString(), "1999-12-31T21:03:20.000Z");
  assert.equal(closedArgs.create.open_time.toISOString(), positionArgs.create.openTime.toISOString());
  assert.equal(closedArgs.create.close_time.toISOString(), positionArgs.create.closeTime.toISOString());
});
...
test("durable mirror writes ack and compatibility cursor together", async () => {
  ...
  const redis = {
    multi() {
      return {
        set(key: string) { calls.push(`set:${key}`); return this; },
        exec() { calls.push("exec"); return Promise.resolve([]); },
      };
    },
  };
  await mirrorHistoryCheckpoint(redis, "123", {
    accountId: "acct-1", phase: "incremental", completedThroughServerTime: "949276800",
    dealsCursor: { time: "949276800", ticket: "10" }, ordersCursor: { time: "949276800", ticket: "20" },
    reconstructionState: null, lastCompletedChunkId: "chunk-1", backfillCompletedAt: "2026-07-13T00:00:00.000Z",
  });
  assert.deepEqual(calls, ["set:mt5:bridge:history-ack:123", "set:mt5:bridge:history-cursor:123", "exec"]);
});
...
test("durable PostgreSQL checkpoint rebuilds both Redis mirrors after Redis flush", async () => {
  ...
  const redis = {
    multi() {
      ...
      return {
        set(key: string, value: string) { writes.push([key, value]); return this; },
        async exec() { for (const [key, value] of writes) values.set(key, value); return []; },
      };
    },
  };
  const checkpoint = {
    accountId: "acct-1", phase: "incremental" as const, completedThroughServerTime: "949276800",
    dealsCursor: { time: "949276700", ticket: "10" }, ordersCursor: { time: "949276710", ticket: "20" },
    reconstructionState: null, lastCompletedChunkId: "chunk-1", backfillCompletedAt: "2026-07-13T00:00:00.000Z",
  };
  ...
  assert.equal(JSON.parse(values.get("mt5:bridge:history-ack:123")!).lastCompletedChunkId, "chunk-1");
