...
test("mapDealPayloadToDeal subtracts the broker's UTC+3 offset when converting server time", () => {
  ...
  const row = mapDealPayloadToDeal("acct-1", {
    ticket: 1, order: 1, positionId: null, symbol: "EURUSD", type: "buy",
    volume: 0.1, price: 1.1, commission: 0, fee: 0, swap: 0, profit: 0,
    time: brokerNoonSeconds, comment: "",
  }, 180);
...
test("mapOrderPayloadToOrder subtracts the broker's UTC+2 offset for timeSetup/timeDone", () => {
  ...
  const row = mapOrderPayloadToOrder("acct-1", {
    ticket: 2, positionId: null, symbol: "EURUSD", type: "buy",
    state: "FILLED", volume: 0.1, priceOpen: 1.1, sl: 0, tp: 0,
    timeSetup: brokerSeconds, timeDone: brokerSeconds, comment: "",
  }, 120);
  assert.ok(row.timeSetup instanceof Date);
  assert.ok(row.timeDone instanceof Date);
  assert.equal((row.timeSetup as Date).toISOString(), "2024-06-01T06:30:00.000Z");
  assert.equal((row.timeDone as Date).toISOString(), "2024-06-01T06:30:00.000Z");
});
...
test("mapPositionClosedPayloadToPosition subtracts the broker's UTC+0 offset as a no-op", () => {
  ...
  const row = mapPositionClosedPayloadToPosition("acct-1", {
    ticket: 779, symbol: "EURUSD", positionType: 0, volume: 0.1,
    entryPrice: 1.1, exitPrice: 1.11, entryTime: seconds,
    exitTime: seconds, durationSeconds: 0, mae: 0, mfe: 0,
    profit: 1, commission: 0, swap: 0, dealTicket: null, orderTicket: null,
    comment: "",
  }, 0);
  assert.ok(row.closeTime instanceof Date);
  assert.ok(row.openTime instanceof Date);
  assert.equal((row.closeTime as Date).toISOString(), "2024-03-10T23:45:00.000Z");
  assert.equal((row.openTime as Date).toISOString(), "2024-03-10T23:45:00.000Z");
});
...
test("mapPositionClosedPayloadToPosition converts entry and exit exactly once for UTC+3 broker", () => {
  ...
  const row = mapPositionClosedPayloadToPosition("acct-1", {
    ticket: 780, symbol: "EURUSD", positionType: 0, volume: 0.1,
    entryPrice: 1.1, exitPrice: 1.11, entryTime: entry, exitTime: exit,
    durationSeconds: 3600, mae: 0, mfe: 1, profit: 1, commission: 0,
    swap: 0, dealTicket: 1, orderTicket: 2, comment: "",
  }, 180);

  assert.equal((row.openTime as Date).toISOString(), "2024-01-01T09:00:00.000Z");
  assert.equal((row.closeTime as Date).toISOString(), "2024-01-01T10:00:00.000Z");
  assert.equal((row.reportDate as Date).toISOString(), "2024-01-01T10:00:00.000Z");
});
