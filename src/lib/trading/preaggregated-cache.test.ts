...
test("buildTradeDistributionDetail accepts PositionRow fixtures from preaggregated-cache", () => {
  ...
  const result = buildTradeDistributionDetail(positions);

  assert.strictEqual(result.available, true);
  if (!result.available) return;
  assert.strictEqual(result.totalPositions, positions.length);

  const nullMaePoint = result.points.find((point) => point.mae === null);
  assert.ok(nullMaePoint);
  assert.strictEqual(nullMaePoint!.mae, null);
  assert.strictEqual(nullMaePoint!.netPnl, -10 + 1 + -0.5);

  const knownPoint = result.points.find((point) => point.mfe === 30.25);
  assert.ok(knownPoint);
  assert.strictEqual(knownPoint!.netPnl, 40 + -2 + -3);
});
