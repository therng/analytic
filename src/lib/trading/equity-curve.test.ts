...
import {
  mergeLiveEquityPoint,
  mapEquitySnapshotRowsToPoints,
  mapEquitySnapshotRowsToDrawdownPercentPoints,
} from "./equity-curve";
...
// --- Equity drawdown percent (Step 7 repair: real equity, not balance) ---

test("mapEquitySnapshotRowsToDrawdownPercentPoints computes percent from persisted peak/drawdown", () => {
  const rows = [
    {
      ts: new Date("2026-07-01T09:00:00.000Z"),
      peakEquity: new Prisma.Decimal(1000),
      drawdown: new Prisma.Decimal(50),
    },
  ];
  const result = mapEquitySnapshotRowsToDrawdownPercentPoints(rows);
  assert.equal(result.length, 1);
  assert.equal(result[0].y, 5);
});

test("mapEquitySnapshotRowsToDrawdownPercentPoints treats a fresh peak (drawdown null) as 0%", () => {
  const rows = [
    {
      ts: new Date("2026-07-01T09:00:00.000Z"),
      peakEquity: new Prisma.Decimal(1000),
...
test("mapEquitySnapshotRowsToDrawdownPercentPoints returns 0% when peakEquity is missing/zero (never divides by zero)", () => {
  const rows = [
    {
      ts: new Date("2026-07-01T09:00:00.000Z"),
      peakEquity: null,
