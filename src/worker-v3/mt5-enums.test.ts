...
test("decodeDealReason maps known codes and returns null for unknown/missing", () => {
  const cases: Array<[unknown, string | null]> = [
    [0, "client"],
    [1, "mobile"],
    [2, "web"],
  ...
  for (const [input, expected] of cases)
    assert.equal(decodeDealReason(input), expected);
});

test("decodeOrderReason maps known codes and returns null for unknown/missing", () => {
  const cases: Array<[unknown, string | null]> = [
    [0, "client"],
    [1, "mobile"],
    [2, "web"],
  ...
  for (const [input, expected] of cases)
    assert.equal(decodeOrderReason(input), expected);
});
