...
export async function GET(request: NextRequest, { params }: AccountRouteContext) {
  ...
  const historyOptions = parsePositionHistoryPageOptions(request.nextUrl.searchParams);

  return withCachedAccountView(request, id, "positions", "Failed to fetch account position details", (payload) =>
    NextResponse.json(paginatePositionsResponse(payload as PositionsResponse, historyOptions)),
  );
}
