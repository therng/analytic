...
**Autonomous card collapse (8.73):** cards self-organize — accounts whose most recent position was **opened within the last 24h** (still open or since closed; `position_opened_recently` on the accounts payload, computed server-side at serialization) render the full card; quiet accounts auto-collapse to a compact strip (`AccountCardStrip`: name/#/status, today growth, equity; name truncated to one line). Size is the scanning signal — no extra badges or rails. There is **no expand/collapse button**: the whole collapsed card is the tap target (`.strip-tap` wraps the strip in `LazyDashboardCard`; keyboard + `focus-visible` preserved) and expansion is **one-way** — tapping pins a session-only expand (no persistence — every reload re-organizes by recent position activity; unpinned cards also re-organize as list refreshes land); collapsing is the autonomous rule's job alone. The strip renders from the accounts-list payload alone (zero per-card requests while collapsed); the expanded card — and the deferred placeholder for activity-expanded cards below the eager slots — reuse the same strip as their header, so header form and figures never change between states. The strip's growth is pinned to TODAY regardless of the body's selected timeframe.
...
| Filled XAUUSD `Order`                  | Deposit-load estimate                                                                                                   |
