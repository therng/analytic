...
**Activity-driven card collapse (8.66):** cards self-organize — accounts that traded today (`today_trade_count > 0`) or hold open positions (`open_position_count > 0`) render the full card; quiet accounts auto-collapse to a compact strip (`AccountCardStrip`: name/#/status + expand chevron inline after the name, today growth, equity; name truncated to one line). Size is the scanning signal — no extra badges or rails. The strip renders from the accounts-list payload alone (zero per-card requests while collapsed); the expanded card — and the deferred placeholder for activity-expanded cards below the eager slots — reuse the same strip as their header, so header form and figures never change between states. The chevron (44px, `aria-expanded`) pins a session-only override (no persistence — every reload re-organizes by today's activity; unpinned cards also re-organize as list refreshes land). The strip's growth is pinned to TODAY regardless of the body's selected timeframe.
