...
- Automatic history lifecycle: Python bridge publishes bounded Deal/Order/Position envelopes with raw MT5 UTC epochs plus barriers; Node worker persists those UTC instants idempotently and advances PostgreSQL `BridgeHistoryCheckpoint` only after all barriers/counts/digests commit. Never shift MetaTrader Python epochs by the broker-server offset. Redis `mt5:bridge:history-ack:{login}` is derived mirror only. Missing state starts at 2000-01-01; never epoch or 30-day fallback.
