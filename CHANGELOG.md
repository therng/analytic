...
- **`bridge/spawn_guard.py`** — classifies the terminal64.exe process-set change at one executable path across an `mt5.initialize()` call (`classify` over `(pid, creation_time)` diffs) and applies the operator-approved response policy (`respond`): a duplicate that appeared while the probed terminal still runs is killed; a replacement (liveupdate/crash-restart relaunch) or plain exit is skipped untouched for the next rescan. Follows the 2026-09 admin-terminal-at-every-boot reports: the MetaTrader5 package may launch a terminal when `initialize()` cannot attach (documented race), and the spawned duplicate inherits the analytic-bridge task's HIGHEST elevation.
- **`bridge/discovery.py`** — every `_discover_one` initialize (success, `False`, or raised) is followed by a watch + diff guard against a **rolling baseline** (the last snapshot taken after the most recent initialize), so only a process appearing across a candidate's own initialize window is attributed to the SDK — never one that appeared during an earlier candidate's processing. After a kill the set is re-watched; a target that survived is named in the warning (`STILL RUNNING after kill … manual intervention required`) instead of being silently absorbed into the next rescan's baseline. Spawn warnings ride the existing discovery-warning surface with slug `unexpected_terminal_launch`, matching `bridge/errors.py`'s `FailureClass.UNEXPECTED_TERMINAL_LAUNCH` vocabulary. `process_killer` plumbed through `discover_accounts` → `resolve_accounts` → `Supervisor` → `__main__`.
- **`RealProcessProbe.watch_candidates()`** (`bridge/adapters/process_probe_psutil.py`) — guard-grade light enumeration (pid/name/exe/create_time only; no cmdline/username PEB reads, the slow path named in the 2026-08-30 postmortem), so the guard's per-candidate re-watch does not multiply the supervisor's full-scan cost.
- **`bridge/adapters/process_killer_psutil.py`** — `PsutilProcessKiller`: terminate→wait→kill ladder with `(pid, creation_time)` re-verification before acting (a recycled PID is never terminated); never raises (outcome string lands in the warning line). Platform truth: on Windows psutil's `terminate()` is an alias for `kill()` — an immediate TerminateProcess with no WM_CLOSE stage — so every guard kill on the production host is forceful, accepted for a seconds-old duplicate with no unsaved state.
- **Tests** — `test_spawn_guard.py` (classification/policy incl. `GuardResponse.kill_targets`), `test_process_killer_psutil.py` (kill ladder, tolerance boundary, exact-pid targeting), `test_discovery.py` `QueueProcessLister` scenarios (duplicate spawn killed+skipped, kill survival flagged, replacement/exit skipped without kill, precedence over initialize failure/raise, other-path ignored, multi-candidate sequencing: spawn during another candidate's window not misattributed, spawn during own window killed, guard kill on first candidate doesn't abort the rest, default-killer fallback executes), and `test_account_resolution.py` passthrough pinning the kill wiring. Bridge suite 437 passed / 4 skipped.
...
### Operations — vps-ops staleness sweep

- Plan: `docs/plans/2026-09-06-vps-ops-staleness-refactor.md`. Every skill file swept against verified live host truth: **SKILL.md** intro/Never-list now name the schtasks bridge control surface (and flag `install-service.ps1` as the retired NSSM path — re-running it as a "repair" would reinstall the old service; the journal-DACL triage in status-summary now uses the direct `icacls` commands instead); **service-install.md** rewritten to the current topology (Redis = WSL systemd + `analytic-redis-wsl-keepalive` task creation, `DependOnService` postgres-only, bridge = `schtasks /Create` of `run-bridge-task.ps1`, verification via `Get-ScheduledTask`); **status-summary.md** services block and log tails name only live control surfaces (`bridge-task.log`, not the 0-byte NSSM-era `bridge-stdout.log`); **deploy.md** full-restart path restarts Redis via WSL systemd; bridge warmup standardized at ~5-6 min everywhere (killed the contradicting 2-3 min claim); **mt5ops.py** routes `postgresql-x64-18` through Stop/Start-Service (the native-service exception) and its `load_accounts` docstring points at `discovered-accounts`. Mirror-synced to `.agents` + hermes destinations per INSTALL.md (which now documents both).
