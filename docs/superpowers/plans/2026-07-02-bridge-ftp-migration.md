...
This task only adds tracking + state persistence to the _existing_ 2s loop — no new streams yet (close-event publishing is Task 5, since it depends on this task's tracker being in place).
...
test("mapPositionClosedPayload maps an enriched close event to a BridgePosition row", () => {
  ...
  assert.equal(
    ...
    new Date(1750999000 * 1000).toISOString(),
  );
  assert.equal(
...
function fakePrisma() {
  ...
  return {
    ...
    bridgeDeal: {
      upsert: (args: unknown) => {
        calls.push({ model: "bridgeDeal", args });
        return Promise.resolve();
      },
    },
    bridgeOrder: {
      upsert: (args: unknown) => {
        calls.push({ model: "bridgeOrder", args });
        return Promise.resolve();
      },
    },
    bridgePosition: {
      upsert: (args: unknown) => {
        calls.push({ model: "bridgePosition", args });
        return Promise.resolve();
      },
    },
  };
...
export async function processStreamEntry(
  ...
  const row = mapPositionClosedPayload(
    tradingAccountId,
  ...
  for (const stream of response) {
    for (const entry of stream.messages) {
      try {
        ...
        console.error(
          `[bridge-consumer] Failed to process ${kind} entry ${entry.id} for ${accountNo}:`,
          error,
        );
      }
    }
  ...
  for (const p of pending) {
    ...
    for (const entry of claimed) {
      try {
        ...
        console.error(
          `[bridge-consumer] Failed to reprocess claimed ${kind} entry ${entry.id} for ${accountNo}:`,
          error,
        );
      }
    }
...
for (const account of accounts) {
  try {
    ...
    const snapshotRow = {
      ...
      drawdown: equityStateRaw
        ? Math.max(0, equityStateRaw.peakEquity - data.live.equity)
        : null,
    };
      ...
      ...(data.positions.length
        ? [
            prisma.openPosition.createMany({
              data: buildOpenPositionRows(account.id, ts, data.positions),
            }),
          ]
        : []),
    ]);
...
```typescript
async function getEquityState(
  accountNo: string,
): Promise<{ peakEquity: number } | null> {
  const redis = await (
    await import("../lib/redis-social")
  ).getRedisSocialClient();
...
async function comparePositions(tradingAccountId: string, accountNo: string) {
  ...
  for (const ftpRow of ftpRows) {
    ...
    const profitDelta = Math.abs(
      Number(ftpRow.profit) - Number(bridgeRow.profit),
    );
    if (profitDelta > PROFIT_TOLERANCE) {
      mismatched.push(
        `${ftpRow.positionNo} (ftp=${ftpRow.profit}, bridge=${bridgeRow.profit})`,
      );
    }
  }
  ...
  console.log(
    `  FTP rows: ${ftpRows.length}, Bridge rows: ${bridgeRows.length}`,
  );
  console.log(
    `  Missing from bridge: ${missing.length ? missing.join(", ") : "none"}`,
  );
  console.log(
    `  Profit mismatches (> $${PROFIT_TOLERANCE}): ${mismatched.length ? mismatched.join(", ") : "none"}`,
  );
...
async function compareDeals(tradingAccountId: string, accountNo: string) {
  ...
  const missing = ftpRows
    .filter((r) => !bridgeByDealNo.has(r.dealNo))
    .map((r) => r.dealNo);
...
async function main() {
  ...
  for (const account of accounts) {
    ...
    totalIssues +=
      posResult.missing + posResult.mismatched + dealResult.missing;
  }
