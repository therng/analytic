...
## Phase 5 (2026-07-15): dual-writer cutover + ClosedPosition reconstruction

**Dual-writer cutover** — resolved by cutting legacy over rather than coordinating two writers. `src/worker/equity-sampler.ts` gates `AccountSnapshot`/`OpenPosition` writes behind `WORKER_ENABLE_LIVE_SYNC` (default off); `docker-compose.yml` flips `WORKER_V2_ENABLE_LIVE_SYNC` default to on. Worker V2's `live-sync.ts` is now the sole writer for those two tables. Committed `f1935a7`.

**Backfill gap — investigated and closed as a non-issue.** `bridge_v2/history_publisher.py` is a deliberately different design from legacy's chunked/barrier backfill (own docstring: "no barriers, no custom ACK... start is configurable; default 2026-01-01, never now-30d, never 2000"). Confirmed empirically: `V2_HISTORY_START` env defaults `2026-01-01` for every account uniformly, and each account's Redis cursor (`mt5:v2:history:{login}:cursor`) had already reached present. The accounts with sparse `Deal` history (`7948784`, `7953093`, `7954220`, `7998410`) simply have no trading activity before 2026-07-14 — bridge_v2 already covers everything that exists. No 2000-01-01 backfill work needed; user confirmed 2026-01-01+ scope is sufficient product-wise.

**ClosedPosition reconstruction — implemented.** `src/worker-v2/position-reconstructor.ts`, wired into `deal-consumer.ts`'s deal handler (triggers after every deal with `direction` in `in`/`out`/`inout`/`out_by`).

Downstream-reader audit (directive: never silently substitute one table for another) found **both** `Position` and `ClosedPosition` are independently authoritative and must both be populated:

- `Position` — read by `calculate-report-results.ts` (win rate/profit factor/Sharpe/`AccountReportResult`) and `account-data.ts` (earliest-open-time windowing).
- `ClosedPosition` — read by `aggregate-performance.ts` (`AccountPerformanceByStrategy`/`BySymbol`, `net_pnl = profit + swap + commission`).

Both are written atomically in one `prisma.$transaction`.

**Position lifecycle algorithm** (pure function `computePositionLifecycle`, unit-tested independent of Prisma): walk a `(tradingAccountId, positionId)` group's `Deal` rows sorted by `(time, dealNo)` — never trust arrival order. Track unsigned open volume + open side. `in` adds to the open side (establishing it if flat). `out`/`out_by` reduce it. `inout` (MT5 reversal) splits its volume at the pre-deal open volume: `min(dealVolume, openVolume)` closes the existing side at the deal's price, the remainder (if any) opens the opposite side at the same price — exact, since MT5 fills a reversal at one price. A position is "closed" only when open volume returns to exactly zero (Decimal, never float equality). Recomputed from scratch from all persisted `Deal` rows on every trigger — replay-safe and out-of-order-safe by construction, no incremental state.

**Schema limitation found and documented, not silently resolved:** if open volume returns to exactly zero and a later deal for the same `positionId` makes it nonzero again, MT5 has reused the position ticket for a second lifecycle. Both `Position` (`@@unique([tradingAccountId, positionNo])`) and `ClosedPosition` (`@@id([account_number, position_id])`) can only hold one row per position ID — this cannot be represented without a schema change. `computePositionLifecycle` returns `{status: "ambiguous-reopen"}` in this case; the caller logs a structured error (`login`, `positionId`, `lastDealNo`) and does not write anything, rather than merging two lifecycles into one wrong row.

**Other gaps found and documented (not blockers, all nullable columns):**

- `Deal` doesn't capture MT5's `magic` or `reason` fields — present in the raw bridge_v2 payload, dropped by `mapDealToPrisma`. Reconstructed `Position.magic`/`ClosedPosition.magic`/`.reason` will be `null` until `Deal` gains columns for them.
- `duration` has no column on either target table (legacy's own `mapPositionClosedPayloadToPosition` didn't persist it either, despite the raw payload carrying `durationSeconds`) — computed in the pure function's result for testability, not persisted.
- `Position.mae`/`.mfe` and `ClosedPosition.mae_money`/`.mfe_money`/`.mae_price`/`.mfe_price`/`.max_drawdown_from_peak_max` require `PositionExcursion` sampling data, out of scope for Deal-only reconstruction — left `null`.
- `sl`/`tp` are not present on `Deal` at all (would need `Order`/live-snapshot data) — left `null`.
- `side` reflects the position's _opening_ side; if a position reverses mid-life, the single `type` column can't represent both sides — documented convention, not a blocker (the row itself is still one correct continuous lifecycle spanning the reversal).

**Changed files:** `src/worker-v2/position-reconstructor.ts` (new), `src/worker-v2/position-reconstructor.test.ts` (new, 11 tests), `src/worker-v2/deal-consumer.ts` (wired trigger).

**Verification:** 93 pre-existing worker-v2 tests + 11 new tests, all pass. `npx tsc --noEmit` introduces zero new errors (10 pre-existing errors in unrelated files, predating this change). `npm run lint` clean (2 pre-existing warnings, unrelated file). `npm run build:worker`, `npm run build:worker-v2`, `npm run build` all succeed.
