...
- `side` reflects the position's *opening* side; if a position reverses mid-life, the single `type` column can't represent both sides — documented convention, not a blocker (the row itself is still one correct continuous lifecycle spanning the reversal).
