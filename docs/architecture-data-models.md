...
Panel-level aggregates (`positions.summary.botPerformance`, `summary.dailyPnl`) are a separate server-side path: computed inside the cached view build by `src/lib/trading/preaggregated/panel-aggregates.ts` (8.61) so `BotPnLPanel` and the P/L heatmap ride the `positions?…&history=0` summary instead of downloading raw positions.
...
### Deleted models (2026-07-19, `a58045f`) — historical record

Four dead models were confirmed via grep (zero `prisma.<model>.*` calls across `src/`, `bridge/`, `scripts/`) and dropped from `schema.prisma` plus back-relations on `TradingAccount`/`OpenPosition`. Recorded here so a future git-archaeology pass doesn't mistake them for load-bearing tables that "went missing":

| Model | Why it was dead | Superseded by |
|---|---|---|
| `Symbol` | Never wired up. No writer ever existed. | — |
| `EquityState` | Early design intended this as equity/drawdown source. Also used raw `snake_case` field names with no `@map`/`@@map`, breaking this schema's naming convention. | `EquitySnapshot` |
| `PositionState` | Early design intended this as MAE/MFE source. Its relation to `OpenPosition` required `account_number` to hold `TradingAccount.id` (cuid) despite every other model using that field name for broker account number. | `Position.mae`/`Position.mfe` + `PositionExcursion` |
| `RiskMetricsSnapshot` | Never wired up on either side. | — |

Follow-up phases of the same metrics migration completed 2026-07-22: `groupBy=symbol|strategy` API branch had zero frontend callers, so `ClosedPosition`, `AccountPerformanceBySymbol`/`AccountPerformanceByStrategy`, `Strategy` (its only consumer was that same branch), and `aggregate-performance.ts` all dropped rather than migrated forward. `Position` now sole closed-trade source, confirmed via real-data parity check (21,378 rows, 4 accounts, 100% match) before drop.

The legacy `BridgeHistoryCheckpoint`/`BridgeHistoryChunk`/`BridgeHistoryRecord` tables (bridge_v2-era PostgreSQL-side history bookkeeping) were dropped with the worker-v3/Compose leanness pass — backfill/coverage bookkeeping is owned entirely by the bridge's SQLite journal (ADR-0005).
