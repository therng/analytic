...
**Single environment (forexvps — Windows Server 2022, single host, dev = prod):** `postgresql-x64-18` (native service, `127.0.0.1:5432`) + Redis **8.0.5 inside WSL2 Ubuntu** behind systemd (`redis-server` unit enabled) held alive by the `analytic-redis-wsl-keepalive` scheduled task (ONLOGON `wsl --exec sleep infinity` — the distro terminates when its last session ends, which is what caused the 2026-08-30 outage) + `analytic-web` + `analytic-worker` + `caddy` (sole public exposure, `https://therng.duckdns.org`) as NSSM services running as LocalSystem, alongside the MT5 terminals and the `bridge` — which runs via the `analytic-bridge` scheduled task (ONLOGON as `analyticvps\supachai` in the console session, `bridge/scripts/run-bridge-task.ps1` behind `-WindowStyle Hidden` — never a wscript launcher: `/End` orphans its children), not NSSM. Data plane is loopback-only. There is no separate local Docker dev stack — the retired `docker-compose.yml` dev topology and `src/worker/`/`src/worker-v3/` runtimes must not be reintroduced. Migration plan: `docs/superpowers/plans/2026-08-17-windows-single-host-migration.md`.
...
- **Hand-rolled SVG token colors** (sparkline, live beacon): pass design-token colors via inline `style` — never `var()` inside presentation attributes (`stroke=`, `fill=`); SVG presentation attributes cannot resolve custom properties, so the element renders invisible with nothing failing (`1b88cd3`)
...
- **Repo subagents** (`.claude/agents/`) — four specialized agents; fan the three reviewers out in one parallel batch on analytics/bridge diffs and synthesize in the parent: `source-boundary-reviewer` (metric source boundaries, net-P/L math, TS-side broker-time), `view-contract-guardian` (view-build fixture/registry/cache-key artifact atomicity), `bridge-reviewer` (Python bridge durability invariants), plus on-demand `test-writer` (repo harness dialect: pure-function + source-contract tests).
