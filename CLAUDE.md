...
- **verify-deploy subagent** (`.claude/agents/verify-deploy.md`) — dispatch it for the post-change verify pipeline (tests → lint → tsc → build with the NSSM stop/start dance → spare-port smoke → docs-sync → scoped docs commit) and for production deploys. Service operations and deploys run only when the dispatch prompt carries a literal `APPROVAL: service-ops` / `APPROVAL: deploy` line; without it the agent stops at the boundary and reports `AWAITING_APPROVAL`. It never pushes, never bumps `package.json` version, never edits source. Dispatch one at a time (build/service steps collide) with mode `acceptEdits`.
