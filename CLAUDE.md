...
**Single environment (forexvps — Windows Server 2022, single host, dev = prod):** `postgresql-x64-18` (native service, `127.0.0.1:5432`) + Redis **8.0.5 inside WSL2 Ubuntu** behind systemd (`redis-server` unit enabled) held alive by the `analytic-redis-wsl-keepalive` scheduled task (ONLOGON `wsl --exec sleep infinity` — the distro terminates when its last session ends, which is what caused the 2026-08-30 outage) + `analytic-web` + `analytic-worker` + `caddy` (sole public exposure, `https://therng.duckdns.org`) as NSSM services running as LocalSystem, alongside the MT5 terminals and the `bridge` NSSM service (must run as `analyticvps\supachai` — LocalSystem cannot attach to the session-1 MT5 terminals; see `bridge/scripts/install-service.ps1`). Data plane is loopback-only. There is no separate local Docker dev stack — the retired `docker-compose.yml` dev topology and `src/worker/`/`src/worker-v3/` runtimes must not be reintroduced. Migration plan: `docs/superpowers/plans/2026-08-17-windows-single-host-migration.md`.
