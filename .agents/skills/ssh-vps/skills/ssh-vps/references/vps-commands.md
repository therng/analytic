...
## 0. SSH connection diagnostics

If `ssh icvps 'whoami'` fails, read the exact error before doing anything else:

- `ssh: connect to host ... port 22: Operation timed out` / `No route to host` — host down or network/firewall blocking. Report to the user, don't retry more than once.
- `Permission denied (publickey)` — key was rotated/removed from the host, or `~/.ssh/config` points at the wrong user/identity file. Report the error verbatim; don't fall back to a password prompt or try other keys.
- `WARNING: REMOTE HOST IDENTIFICATION HAS CHANGED` — the host's SSH key no longer matches `known_hosts`. This means the VPS was rebuilt/reimaged, or something is intercepting the connection. Stop and confirm with the user before editing `known_hosts` or adding `-o StrictHostKeyChecking=no` — never do this silently.
- Command hangs with no output — usually a PowerShell quoting mistake (missing `-NoProfile`, unclosed quote). Compare against the tested examples below rather than guessing new quoting.
