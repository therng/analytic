...
TEMP OPEN:
`ssh forexvps 'powershell -NoProfile -Command "Start-Process \"$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\<Name>.lnk\""'`
...
2. `ssh forexvps 'powershell -NoProfile -Command "New-Item -ItemType Directory -Force -Path C:\Pause | Out-Null; Move-Item \"$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\<Name>.lnk\" -Destination C:\Pause -Force"'`
Confirm name → exactly 1 shortcut before acting; ask if ambiguous.

RESUME:
`ssh forexvps 'powershell -NoProfile -Command "Move-Item \"C:\Pause\<Name>.lnk\" -Destination \"$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\" -Force; Start-Process \"$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\<Name>.lnk\""'`
