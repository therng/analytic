...
description: Review cross-cutting design decisions, ADRs, and structural changes against docs/architecture-data-models.md, docs/harness/analytic/team-spec.md, and incident/ADR records. Use for changes that shift ownership boundaries (e.g. worker responsibilities, data-source authority, checkpoint ownership) or that need a new ADR. Not for a single-domain formula/UI/ingestion review — route those to the matching domain reviewer instead.
...
- Check existing ADRs and incident records in `docs/` for precedent before a repeat decision gets re-litigated from scratch.
