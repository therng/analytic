...
- Production is a SINGLE Windows host: everything runs natively on `forexvps` (Windows Server 2022) — MT5 terminals + `bridge` (NSSM) + `redis-wsl` (WSL2, 127.0.0.1:6379) + `postgresql-x64-18` (127.0.0.1:5432) + `analytic-worker` (health 127.0.0.1:9200) + `analytic-web` (127.0.0.1:3000) + `caddy` (80/443). The local Mac `docker compose` stack is dev-only and never serves therng.duckdns.org. On-host probes: `.claude/skills/ssh-vps/references/analytic-services.md` (stack) and `status-check.md` (bridge/terminals).
