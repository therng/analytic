...
- After infra changes, verify with `docker compose up -d` locally where feasible, then `npm run build` and `GET /api/accounts` (the static `/api/health` proves nothing about DB/Redis; the pipeline probe is worker-v2 `:9200/health`).
