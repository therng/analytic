...
- `scripts/check-harness-review.sh` runs as a pre-push hook (`npm run hooks:install` once per clone; `npm run harness:check` ad hoc). It blocks a push that:
  - touches an ingestion/analytics/dashboard domain path (per `docs/harness/analytic/team-spec.md` routing table) unless a commit in the push pairs `<domain> review: pass` with a domain-path diff, or `_workspace/02_review_{domain}.md` was added/updated in the push (stale committed artifacts never count; suffixed records go under `_workspace/review-log/`).
  - adds a hardcoded `REDIS_PASSWORD`/`DATABASE_URL`/`DUCKDNS_TOKEN` literal or a stray `.env*` file (`.env.example`/`.env.test.example` are allowed).
...
- Commit messages end with the trailer `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>`; PR bodies end with the line `🤖 Generated with [Claude Code](https://claude.com/claude-code)`.
