...
3. Stack services (auto-start via SCM + DependOnService — data tier first):
   `ssh forexvps 'powershell -NoProfile -Command "Get-Service postgresql-x64-18,redis-wsl,analytic-worker,analytic-web,caddy | Format-Table Name,Status"'`
   Expect all Running within ~2 min of boot (worker may cycle a few times while Redis/Postgres warm up — that's normal). Stuck STOPPED/START_PENDING > 2 min → analytic-services.md for per-service checks; `no such service` = real fault — see analytic-services.md.
