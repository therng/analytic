...
6. **Journal dir ACL must be protected, not inherited.** `bridge/journal/connection.py::_validate_windows_acl` requires the journal directory's DACL to have `SE_DACL_PROTECTED` set (inheritance broken) and every ACE to name only the running service account's SID — an inherited ACL (the default when the dir is freshly created under `C:\analytic`, which grants SYSTEM/Administrators/Users) always fails this check. Failure looks like a false `journal_failure` quarantine (`"journal parent must be a real directory"` or `"journal path must be restricted to the service identity"`) even though the directory exists and is readable. `install-service.ps1` applies `icacls state\journal /inheritance:r /grant:r <account>:(OI)(CI)F /setowner <account>` every run — don't skip this step if repairing by hand, and re-apply it after any manual delete/recreate of `state\journal` (e.g. via `history:reset` or a fresh clone).
...
6. Confirm `state\journal` ACL is protected + restricted to the service account only (`icacls C:\analytic\bridge\state\journal` — every entry should be non-inherited and name only the service account, no `(I)` flags). If not, re-run `install-service.ps1` or apply manually per preflight item 6 above, then clear any stale quarantine files (`state\quarantine\*.json`) before restarting.
7. `ssh forexvps 'nssm start bridge'`, then verify per status-check.md (health JSON under state dir, not the old Redis heartbeat key).
