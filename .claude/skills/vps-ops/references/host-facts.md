...
| `bridge` | Scheduled task `analytic-bridge` (ONLOGON, RU `analyticvps\supachai`, HIGHEST; NSSM variant retired 2026-08-30) | `wscript.exe C:\analytic\bridge\scripts\run-bridge-task-hidden.vbs` (hidden console — no window at logon; vbs waits so the task stays Running and `/End` still kills the tree) → `powershell -File C:\analytic\bridge\scripts\run-bridge-task.ps1` → `C:\Python314\python.exe -m bridge`; runs in the console session (session 1) as the terminal-owning user; supervisor spawns one worker child per account; restart = `schtasks /End /TN analytic-bridge && schtasks /Run /TN analytic-bridge` (never NSSM; ~5-6 min warmup before live TTLs republish) | — (outbound Redis only) | `C:\analytic\bridge\logs\bridge-task.log` (wrapper tees stdout/stderr; worker output is block-buffered — check redis TTLs + `state\health\` for liveness, not logs) |
