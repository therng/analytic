...
| `bridge` | Scheduled task `analytic-bridge` (ONLOGON, RU `analyticvps\supachai`, HIGHEST; NSSM variant retired 2026-08-30) | `powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File C:\analytic\bridge\scripts\run-bridge-task.ps1` → `C:\Python314\python.exe -m bridge`; hidden console (brief flash at logon; wscript launcher rejected 2026-09-07 — `/End` killed only the wscript head, children orphaned); runs in the console session (session 1) as the terminal-owning user; supervisor spawns one worker child per account; restart = `schtasks /End /TN analytic-bridge && schtasks /Run /TN analytic-bridge` (never NSSM; ~5-6 min warmup before live TTLs republish) | — (outbound Redis only) | `C:\analytic\bridge\logs\bridge-task.log` (wrapper tees stdout/stderr; worker output is block-buffered — check redis TTLs + `state\health\` for liveness, not logs) |
| `analytic-mt5-update-watch` | Scheduled task (ONLOGON +PT2M delay + hourly repeat, RU `analyticvps\supachai` HIGHEST, hidden powershell, Interactive only) | `powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File C:\analytic\.claude\skills\vps-ops\scripts\mt5update.ps1 -Mode Watch -Confirm -Reboot -Quiet` — applies fully-staged MetaQuotes builds to every Startup-folder terminal (signature-gated, per-terminal backups) and reboots; self-heals hung `/update` copiers (>15 min) and purges stale liveupdate staging when the fleet is up-to-date; a FAILED marker in the log dir suppresses auto-runs | — | `C:\analytic\logs\mt5update\mt5update.log` |
