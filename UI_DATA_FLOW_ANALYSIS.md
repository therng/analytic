...
| **MAE / MFE Display** | *N/A* | *N/A* | *N/A* | `PositionState` / `ClosedPosition` | **Missing Feature**: MAE/MFE data is processed by the backend but not displayed in the UI. | Create a new UI component to display MAE/MFE data for closed trades, fetching from the `ClosedPosition` table. |
...
| **Bridge/Debug Status** | *Not Found* | *Not Found* | *Not Found* | Redis keys + streams | **Missing Feature/Info**: Could not locate a specific bridge/debug status page in the codebase. | Clarify if this page exists or needs to be created. If so, it should fetch data directly from Redis admin commands (`KEYS`, `XINFO`, etc.) via a dedicated API endpoint. |
