...
bridge-only mode; manual local HTML import remains available for one-off backfill/debug.
...
- [ ] Cleanup follow-up: decide whether to keep `src/lib/parser/` for manual local backfill,
      and remove the `Bridge*` shadow tables when no longer needed
